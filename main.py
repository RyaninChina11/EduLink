import time, datetime, requests, json, TTS, logging, random, os, siot
from requests.exceptions import RequestException
from unihiker import GUI, Audio
siot.init(client_id="3Q_H7J1Hg",server="iot.dfrobot.com.cn",port=1883,user="_O64nJ1Hg",password="ld6V7J1HRz")
siot.connect()
siot.loop()
siot.getsubscribe(topic="3Q_H7J1Hg")
audio, gui = Audio(), GUI()
    ...
    try:
        ...
        for course in daily_courses:
            ...
            if start <= current_time <= end:
                return format_course_output(course, current_weekday)
        return format_no_class_output(current_weekday)
    except KeyError:
...
def format_course_output(course, weekday):
    return (
        f"{course['name']}\n"
        f"{course['start']} - {course['end']}"
    )

def format_no_class_output(weekday):
    return "当前没有课程\n"
